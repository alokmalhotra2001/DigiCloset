...
public struct CurrentWeatherResponse: Decodable {
    ...
    static func mock() -> CurrentWeatherResponse {
        CurrentWeatherResponse(coordinate: Coordinate(latitude: 0.0, longitude: 0.0), weathers: [WeatherInfo(id: 0, main: WeatherCondition.snow, description: "It's snowing", icon: "04d")], name: "Holmdel", data: WeatherData(temp: 32.0, feelsLike: 30.0, humidity: 30, low: 20.0, high: 45.0), wind: Wind(speed: 5.0, deg: 6, gust: 10.0), clouds: Clouds(coverage: 50.0))
    }
}
...
public struct ForecastSummary: Decodable, Hashable {
    ...
    static func mockCollection() -> [ForecastSummary] {
        [ForecastSummary(data: WeatherData(temp: 60.0, feelsLike: 65.0, humidity: 20, low: 40.0, high: 80.0), weather: [WeatherInfo(id: 1, main: .clear, description: "Clear", icon: "Sun")], clouds: Clouds(coverage: 0.0), wind: Wind(speed: 10.0, deg: 5, gust: 20.0), visibility: 5, threeHourChanceOfRain: 20.0, date: Date()),
       ForecastSummary(data: WeatherData(temp: 20.0, feelsLike: 10.0, humidity: 20, low: 10.0, high: 30.0), weather: [WeatherInfo(id: 1, main: .clouds, description: "Cloudy", icon: "Cloud")], clouds: Clouds(coverage: 80.0), wind: Wind(speed: 10.0, deg: 5, gust: 20.0), visibility: 5, threeHourChanceOfRain: 20.0, date: Date())
      ]
    }
  }
...
extension ForecastSummary {
    ...
    public init(from decoder: Decoder) throws {
        ...
        if let rainContainer = try? values.nestedContainer(keyedBy: Rain.CodingKeys.self, forKey: .rain),
           let rainChance = try rainContainer.decodeIfPresent(Double.self, forKey: .threeHourChance) { threeHourChanceOfRain = rainChance }
        else { self.threeHourChanceOfRain = 0 }
    }
